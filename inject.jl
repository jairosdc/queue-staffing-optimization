...
# The new python code we want to inject
new_code = """
import numpy as np
import pandas as pd
import heapq
...
def simular_jornada(lambda_base: float, pesos: np.ndarray, mus: np.ndarray, 
    ...
    for i, llegada in enumerate(llegadas):
        ...
        hora_actual = min(len(pesos) - 1, Int(floor(inicio_servicio)))
...
try
    # Read the notebook
    filepath = "Analisis_de_dataset.ipynb"
    nb = JSON.parsefile(filepath)

    # Split the python code by lines, appending \n
    lines = split(new_code, "\n")
    source_array = [line * "\n" for line in lines[1:end-1]]
    if length(lines) > 0
        push!(source_array, lines[end])
    end

    found = false
    # Iterate through cells to find the correct one
    for cell in nb["cells"]
        if cell["cell_type"] == "code"
            source_text = join(cell["source"])
            if occursin("def simular_jornada", source_text) && occursin("def registrar_experimento", source_text)
                cell["source"] = source_array
                found = true
                println("Celda encontrada y modificada")
                break
            end
        end
    end
    ...
    if found
        ...
        println("No se encontró la celda objetivo.")
    end
catch e
